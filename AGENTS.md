...
## Overview
clnkr is a coding agent CLI that queries LLMs and executes bash commands in a loop. Core library is stdlib only. Talks to the Anthropic Messages API and any OpenAI-compatible endpoint. Ships two binaries: `clnku` (plain CLI) and `clnkr` (TUI frontend). Evals run through the standalone `clankerval` project.
...
## Rules
- No external deps in root go.mod. Stdlib only. Charm deps live only in `cmd/clnkr/go.mod`.
...
- Adapter tests use external packages (`package anthropic_test`). Core tests use internal (`package clnkr`). TUI tests use internal (`package main`). Match the existing pattern.
...
- Do not assume `clnkr` and `clnku` need feature parity. Keep `cmd/clnku` minimal unless the task calls for both.
...
## Architecture
Core importable library at module root. Two CLI consumers. `evaluations/` consumed by external `clankerval` runner.
```
...
├── cmd/clnkr/          # TUI (own go.mod, charm v2 deps)
└── evaluations/        # Eval suites for clankerval
```
`cmd/clnkr/` has its own `go.mod`. Root `go test ./...` skips it — use `make test`. `go install .../cmd/clnkr@latest` fails due to replace directive; use `make build` or releases.
...
- TUI: inject via narrowest local interface, not concrete structs
