...
`make check` also enforces repo-local architecture edges via `_arch` and import-graph core SLOC accounting via `sloc`. Transitional deferred-package imports live in `scripts/deferred-package-allowlist.txt`.
