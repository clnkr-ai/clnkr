...
func TestWriteEventLogWorkingMemoryUpdatedPayload(t *testing.T) {
	f, err := os.CreateTemp("", "clnkr-event-log")
	if err != nil {
		t.Fatal(err)
	}
	...
	if err := WriteEventLog(f, clnkr.EventWorkingMemoryUpdated{
		Reason: "command",
		Stats:  clnkr.WorkingMemoryStats{PreviousBytes: 10, UpdatedBytes: 20, DeltaMessages: 3},
	}); err != nil {
		t.Fatalf("WriteEventLog: %v", err)
	}
	...
	var payload struct {
		...
		Payload struct {
			...
			Stats  struct {
				...
				UpdatedBytes  int  `json:"updated_bytes"`
	...
	if payload.Type != "working_memory_updated" || payload.Payload.Reason != "command" {
		t.Fatalf("payload = %#v, want working_memory_updated command", payload)
	}
	if payload.Payload.Stats.PreviousBytes != 10 || payload.Payload.Stats.UpdatedBytes != 20 || payload.Payload.Stats.DeltaMessages != 3 || payload.Payload.Stats.Rejected {
		t.Fatalf("stats = %#v, want explicit working memory stats", payload.Payload.Stats)
	}
}
