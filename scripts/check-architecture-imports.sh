...
check_edge() {
  ...
  case "$kind" in
    ...
    frontend_runtime) [[ "$target" == "$module" ]] || { echo "error: $importer -> $target: internal frontend runtime packages should keep repo-local imports to root clnkr only" >&2; return 1; } ;;
    provider)
      case "$target" in
