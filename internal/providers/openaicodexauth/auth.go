...
type fileData map[string]Token
...
func DeviceLogin(ctx context.Context, w io.Writer, cfg Config) error {
	...
	if err := writeDevicePrompt(w, device); err != nil {
		return err
	}
...
func DeviceLoginFromEnv(ctx context.Context, w io.Writer, env func(string) string) error {
	return DeviceLogin(ctx, w, Config{
		AuthBaseURL: env("CLNKR_OPENAI_CODEX_AUTH_BASE_URL"),
		AuthPath:    env("CLNKR_OPENAI_CODEX_AUTH_PATH"),
	})
}

func RunDeviceLoginFromEnv(w io.Writer, env func(string) string) error {
	ctx, stop := signal.NotifyContext(context.Background(), os.Interrupt)
	defer stop()
	return DeviceLoginFromEnv(ctx, w, env)
}

func RunValidatedDeviceLogin(flags *flag.FlagSet, w io.Writer, env func(string) string) error {
	if err := ValidateLoginFlags(flags); err != nil {
		return err
	}
	return RunDeviceLoginFromEnv(w, env)
}

func RunValidatedDeviceLoginAndExit(flags *flag.FlagSet, w io.Writer, env func(string) string) error {
	if err := RunValidatedDeviceLogin(flags, w, env); err != nil {
		return err
	}
	os.Exit(0)
	return nil
...
func RunValidatedDeviceLoginIfRequested(selected bool, flags *flag.FlagSet, w io.Writer, env func(string) string, fatalf func(string, ...any)) {
	if !selected {
		return
	}
	if err := RunValidatedDeviceLoginAndExit(flags, w, env); err != nil {
		fatalf("%v", err)
	}
}

func ValidateLoginFlags(flags *flag.FlagSet) error {
	var conflict string
...
func writeDevicePrompt(w io.Writer, device deviceCode) error {
	lines := []string{
		"",
...
func requestDeviceCode(ctx context.Context, cfg Config) (deviceCode, error) {
	endpoint := cfg.authURL("/api/accounts/deviceauth/usercode")
	...
	return deviceCode{
		...
		Interval:        deviceInterval(wire.Interval),
	}, nil
}

func deviceInterval(value any) int {
	switch v := value.(type) {
	case float64:
		return int(v)
	case string:
...
func pollDeviceCode(ctx context.Context, cfg Config, device deviceCode) (codeResponse, error) {
	endpoint := cfg.authURL("/api/accounts/deviceauth/token")
	...
	for {
		...
		if resp.StatusCode >= 200 && resp.StatusCode <= 299 {
			...
			return codeResponse{AuthorizationCode: wire.AuthorizationCode, CodeVerifier: wire.CodeVerifier, CodeChallenge: wire.CodeChallenge}, nil
		}
...
func postToken(ctx context.Context, cfg Config, form url.Values) (Token, error) {
	req, err := http.NewRequestWithContext(ctx, http.MethodPost, cfg.authURL("/oauth/token"), strings.NewReader(form.Encode()))
	if err != nil {
		return Token{}, err
	}
	...
	accountID, _ := accountIDFromTokenPair(wire.IDToken, wire.AccessToken)
...
func accountIDFromTokenPair(idToken, accessToken string) (string, error) {
...
func (c Config) authURL(path string) string {
